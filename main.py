...
def pass_signal(row, st):
    typ = st["sinyal"]
    pct = st["sinyal_pct"]
    dist = st["mesafe"]
    n30, n100 = st["n30"], st["n100"]
    h_odd = float(row["H"]) if pd.notna(row.get("H")) else None
    o25 = float(row["O25"]) if pd.notna(row.get("O25")) else None

    if dist >= DIST_MAX:
        return False, "DROP_DIST"

    if typ == "Home":
        if pct < MIN_HOME_AWAY:
            return False, "DROP_PCT"
        if n30["H"] < MIN_WINDOW_1X2 or n100["H"] < MIN_WINDOW_1X2:
            return False, "DROP_WINDOW"
        if abs(n30["H"] - n100["H"]) >= SPLIT_MAX:
            return False, "DROP_SPLIT"
        if h_odd is None or h_odd < HOME_ODDS_MIN:
            return False, "DROP_SHORT"
        if h_odd <= HOME_SHORT_BAND_MAX and (
            n100["H"] < HOME_SHORT_N100 or dist >= HOME_SHORT_DIST
        ):
    ...
    if typ == "Away":
        if pct < MIN_HOME_AWAY:
            return False, "DROP_PCT"
        if n30["A"] < MIN_WINDOW_1X2 or n100["A"] < MIN_WINDOW_1X2:
            return False, "DROP_WINDOW"
        if abs(n30["A"] - n100["A"]) >= SPLIT_MAX:
            return False, "DROP_SPLIT"
        return True, "PASS"
    ...
    if typ == "BTTS":
        ...
        if pct < MIN_BTTS:
            return False, "DROP_PCT"
        if n30["B"] < MIN_BTTS_N30 or n100["B"] < MIN_BTTS_N100:
            return False, "DROP_BTTS_N"
        if abs(n30["B"] - n100["B"]) >= SPLIT_MAX:
            return False, "DROP_SPLIT"
        return True, "PASS"
...
def scan_signals(hist):
    ...
    lines = [
        ...
        f"Taranan: {len(fx)} maç | gönderilen: {len(blocks)}",
        f"kaynak: {', '.join(srcs)}",
